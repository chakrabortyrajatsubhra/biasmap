...
def generate_with_ovam_bias_mitigation(
    ...
    attribution_prompt="A photo of the face of a firefighter and race",
    semantic_token="firefighter",
    demographic_token="race",
    ...
    if save_visualization and original_attn_maps is not None:
        ...
        if mitigated_attn_maps is not None:
        ...
        # Difflens-style 3x2: Unfair|Fair, rows = profession mask, demographic mask, overlap + IoU
        if mitigated_attn_maps is not None:
            difflens_path = output_path.replace(".png", "_difflens_style.png") if output_path else "bias_mitigation_difflens_style.png"
            visualize_bias_mitigation_difflens_style(
                original_image,
                mitigated_image,
                original_attn_maps,
                mitigated_attn_maps,
                ...
                original_iou,
                mitigated_iou,
                output_path=difflens_path,
            )
        # 4-panel mask viz: green=profession/semantic, orange=gender/demographic, yellow=overlap
        if output_path:
            four_panel_path = output_path.replace(".png", "_four_panels.png")
        else:
            four_panel_path = "mask_visualization_four_panels.png"
        visualize_masks_four_panels(
            original_image,
            original_attn_maps["sem_mask"],
            original_attn_maps["demo_mask"],
            original_attn_maps["overlap"],
            semantic_label=semantic_token,
            demographic_label=demographic_token,
...
def _resize_mask_to_image(mask, image):
    """Resize mask to PIL image size. Returns float32 array (H, W)."""
    from PIL import Image as PILImage
    img_arr = np.array(image)
    h, w = (img_arr.shape[0], img_arr.shape[1]) if img_arr.ndim >= 2 else (512, 512)
    if mask.shape[0] == h and mask.shape[1] == w:
        return mask.astype(np.float32)
    m_pil = PILImage.fromarray((np.clip(mask * 255, 0, 255).astype(np.uint8)))
    m_pil = m_pil.resize((w, h), PILImage.NEAREST)
    return (np.array(m_pil) / 255.0).astype(np.float32)


def visualize_bias_mitigation_difflens_style(
    original_image,
    mitigated_image,
    original_attn_maps,
    mitigated_attn_maps,
    ...
    original_iou,
    mitigated_iou,
    output_path="bias_mitigation_difflens_style.png",
):
    """
    Difflens-style 3x2 grid: Unfair (original) | Fair (mitigated).
    Rows: profession (green) mask, demographic (orange) mask, overlap + IoU.
    """
    try:
        # Resize masks to image size
        orig_prof = _resize_mask_to_image(original_attn_maps["sem_mask"], original_image)
        orig_demo = _resize_mask_to_image(original_attn_maps["demo_mask"], original_image)
        fair_prof = _resize_mask_to_image(mitigated_attn_maps["sem_mask"], mitigated_image)
        fair_demo = _resize_mask_to_image(mitigated_attn_maps["demo_mask"], mitigated_image)

        fig, axes = plt.subplots(3, 2, figsize=(12, 18))

        # Row 0: profession (green) mask
        green_unfair = np.zeros((*orig_prof.shape, 3))
        green_unfair[..., 1] = orig_prof
        axes[0, 0].imshow(original_image)
        axes[0, 0].imshow(green_unfair, alpha=0.7)
        axes[0, 0].set_title(f"Unfair Model: '{semantic_token}' Mask")
        axes[0, 0].axis("off")

        green_fair = np.zeros((*fair_prof.shape, 3))
        green_fair[..., 1] = fair_prof
        axes[0, 1].imshow(mitigated_image)
        axes[0, 1].imshow(green_fair, alpha=0.7)
        axes[0, 1].set_title(f"Fair Model: '{semantic_token}' Mask")
        axes[0, 1].axis("off")

        # Row 1: demographic mask (orange for gender, red for race)
        if demographic_token.lower() == "gender":
            demo_unfair = np.zeros((*orig_demo.shape, 3))
            demo_unfair[..., 0] = orig_demo
            demo_unfair[..., 1] = orig_demo * 0.5
            demo_unfair[..., 2] = orig_demo * 0.3
            demo_fair = np.zeros((*fair_demo.shape, 3))
            demo_fair[..., 0] = fair_demo
            demo_fair[..., 1] = fair_demo * 0.5
            demo_fair[..., 2] = fair_demo * 0.3
        else:
            # race or other: red
            demo_unfair = np.zeros((*orig_demo.shape, 3))
            demo_unfair[..., 0] = orig_demo
        ...
        axes[1, 0].imshow(demo_unfair, alpha=0.7)
        axes[1, 0].set_title(f"Unfair Model: '{demographic_token}' Mask")
        axes[1, 0].axis("off")

        axes[1, 1].imshow(mitigated_image)
        axes[1, 1].imshow(demo_fair, alpha=0.7)
        axes[1, 1].set_title(f"Fair Model: '{demographic_token}' Mask")
        axes[1, 1].axis("off")

        # Row 2: Overlap (difflens formula) + IoU
        overlap_unfair = np.zeros((*orig_prof.shape, 3))
        if demographic_token.lower() == "gender":
            overlap_unfair[..., 0] = orig_demo
            overlap_unfair[..., 1] = np.maximum(orig_demo * 0.5, orig_prof)
            overlap_unfair[..., 2] = orig_demo * 0.3
        else:
            # race: red for demo, green for prof
            overlap_unfair[..., 0] = orig_demo
            overlap_unfair[..., 1] = orig_prof
        axes[2, 0].imshow(original_image)
        axes[2, 0].imshow(overlap_unfair, alpha=0.7)
        axes[2, 0].set_title(f"Unfair Model: Overlap\nIoU: {original_iou:.4f}")
        axes[2, 0].axis("off")

        overlap_fair = np.zeros((*fair_prof.shape, 3))
        if demographic_token.lower() == "gender":
            overlap_fair[..., 0] = fair_demo
            overlap_fair[..., 1] = np.maximum(fair_demo * 0.5, fair_prof)
            overlap_fair[..., 2] = fair_demo * 0.3
        else:
            overlap_fair[..., 0] = fair_demo
            overlap_fair[..., 1] = fair_prof
        axes[2, 1].imshow(mitigated_image)
        axes[2, 1].imshow(overlap_fair, alpha=0.7)
        axes[2, 1].set_title(f"Fair Model: Overlap\nIoU: {mitigated_iou:.4f}")
        axes[2, 1].axis("off")

        plt.tight_layout()
        plt.savefig(output_path, dpi=300)
        plt.close()
        print(f"Difflens-style visualization saved to {output_path}")
    except Exception as e:
        print(f"Error creating difflens-style visualization: {e}")
        import traceback
        traceback.print_exc()


def visualize_masks_four_panels(
    image,
    sem_mask,
    demo_mask,
    overlap,
    semantic_label="profession",
    demographic_label="gender",
    output_path="mask_visualization_four_panels.png",
):
    """
    Create 4-panel visualization: Image | Profession (green) | Gender (orange) | Overlap (yellow).
    Matches the style where green = semantic/profession, orange = demographic/gender, yellow = overlap.
    """
    try:
        img_arr = np.array(image)
        if img_arr.ndim == 2:
        ...
        def to_size(m, h_out, w_out):
            if m.shape[0] == h_out and m.shape[1] == w_out:
                return m.astype(np.float32)
            from PIL import Image as PILImage
            m_pil = PILImage.fromarray((np.clip(m * 255, 0, 255).astype(np.uint8)))
            m_pil = m_pil.resize((w_out, h_out), PILImage.NEAREST)
            return (np.array(m_pil) / 255.0).astype(np.float32)

        sem = to_size(sem_mask, h, w)
        demo = to_size(demo_mask, h, w)

        # Compute regions from resized masks so they are mutually exclusive
        # Use threshold 0.5 so resized float masks become binary
        sem_bin = (sem > 0.5).astype(np.float32)
        demo_bin = (demo > 0.5).astype(np.float32)
        overlap_both = sem_bin * demo_bin
        prof_only = sem_bin * (1.0 - demo_bin)
        demo_only = demo_bin * (1.0 - sem_bin)

        # More vivid colors: higher alpha and full saturation
        color_alpha = 0.82  # higher = more vivid (0.5 = subtle, 0.85 = very vivid)

        # Green overlay (profession): bright green
        green_overlay = np.zeros((h, w, 4))
        green_overlay[..., 1] = prof_only
        green_overlay[..., 3] = color_alpha * prof_only

        # Demographic overlay: orange for gender, red for race
        demo_overlay = np.zeros((h, w, 4))
        demo_overlay[..., 0] = demo_only
        if demographic_label.lower() == "gender":
            demo_overlay[..., 1] = 0.5 * demo_only
            demo_overlay[..., 2] = 0.3 * demo_only
        # else race: pure red (G and B already 0)
        demo_overlay[..., 3] = color_alpha * demo_only

        # Yellow overlay (overlap): bright yellow
        yellow_overlay = np.zeros((h, w, 4))
        yellow_overlay[..., 0] = overlap_both
        yellow_overlay[..., 1] = overlap_both
        yellow_overlay[..., 3] = color_alpha * overlap_both

        # Darkened base for mask panels: semi-transparent black on top so colors pop
        img_float = img_arr.astype(np.float32) / 255.0 if img_arr.max() > 1 else img_arr.astype(np.float32)
        darken = 0.85  # blend with black (0.55 = 45% image, 55% black)
        darkened_base = (img_float * (1.0 - darken)).clip(0, 1)

        demo_color = "red" if demographic_label.lower() == "race" else "orange"
        fig, axs = plt.subplots(1, 4, figsize=(16, 4))
        fig.suptitle(f"Mask visualization: {semantic_label} (green), {demographic_label} ({demo_color}), overlap (yellow)", fontsize=12)
        ...
        # Panel 2: Profession (green) — darkened base so green stands out
        axs[1].imshow(darkened_base)
        axs[1].imshow(green_overlay)
        axs[1].set_title(semantic_label.capitalize())
        axs[1].axis("off")

        # Panel 3: Demographic (orange or red) — darkened base
        axs[2].imshow(darkened_base)
        axs[2].imshow(demo_overlay)
        axs[2].set_title(demographic_label.capitalize())
        axs[2].axis("off")

        # Panel 4: Overlap — darkened base, then green + demographic + yellow
        axs[3].imshow(darkened_base)
        axs[3].imshow(green_overlay)
        axs[3].imshow(demo_overlay)
        axs[3].imshow(yellow_overlay)
        ...
        plt.savefig(output_path, dpi=300)
        plt.close()
        print(f"Four-panel mask visualization saved to {output_path}")
    except Exception as e:
        print(f"Error creating four-panel visualization: {e}")
        import traceback
        traceback.print_exc()
...
def run_single_example(
    ...
    """
    Run one example with bias mitigation.

    Args:
        profession: e.g. "architect", "musician", "doctor", "mechanic"
        seed: random seed
        model_id: "runwayml/stable-diffusion-v1-5" (SD 1.5) or "stabilityai/stable-diffusion-2-base" (SD 2.0)
        attribution: "gender" (male->female, orange mask) or "race" (white<->African-American, red mask)
        scene: e.g. "kitchen", "office" - if provided, prompt becomes "A photo of a {profession} in a {scene}"
    """
    if output_dir is None:
        scene_suffix = f"_{scene}" if scene else ""
        output_dir = f"{profession}{scene_suffix}_{attribution}_example"
    os.makedirs(output_dir, exist_ok=True)

    if scene:
        generation_prompt = f"A photo of a {profession} in a {scene}"
        attribution_prompt = f"A photo of a {profession} in a {scene} and {attribution}"
    else:
        generation_prompt = f"A photo of the face of a {profession}"
        attribution_prompt = f"A photo of the face of a {profession} and {attribution}"
    
    semantic_token = profession
    demographic_token = attribution
    if attribution.lower() == "race":
        editing_prompt = ["a white person", "an African-american person"]
    else:
        editing_prompt = ["a man", "a woman"]  # male to female
    ...
    print(f"\n=== Single example: {profession}, seed {seed}, attribution: {attribution} ({model_id}) ===")
    results = generate_with_ovam_bias_mitigation(
        ...
        demographic_token=demographic_token,
        save_visualization=True,
        output_path=os.path.join(output_dir, "bias_mitigation_result.png"),
        editing_prompt=editing_prompt,
        seed=seed,
        guidance_scale=guidance_scale,
        num_inference_steps=num_inference_steps,
    ...
    # Save images
    results["original_image"].save(os.path.join(output_dir, f"original_{profession}.png"))
    results["mitigated_image"].save(os.path.join(output_dir, f"mitigated_{profession}.png"))

    # Four-panel mask viz is already saved by generate_with_ovam_bias_mitigation as bias_mitigation_result_four_panels.png
    four_panel_src = os.path.join(output_dir, "bias_mitigation_result_four_panels.png")
    if os.path.isfile(four_panel_src):
        import shutil
        shutil.copy(
            four_panel_src,
            os.path.join(output_dir, f"mask_visualization_{profession}_{demographic_token}.png"),
        )
...
def run_single_example_architect(
    output_dir="architect_male_to_female_example",
    seed=455678,
    guidance_scale=7.0,
    num_inference_steps=50,
    regressor_scale=100.0,
    ...
    """Convenience wrapper: architect, SD 1.5, seed 455678."""
    ...
    for profession in professions:
        for seed in seeds:
            ...
            if seed % 2 == 0:
                ...
                editing_prompt = ['an African-american person', 'a white person']
            generation_prompt = f"A photo of the face of a {profession}"
            attribution_prompt = f"A photo of the face of a {profession} and race"
            semantic_token = profession
            demographic_token = "race"
...
if __name__ == "__main__":
    import sys
    # Optional second arg: "sd15" or "sd2" to pick model (default: sd15 for musician/pilot)
    model_arg = sys.argv[2].lower() if len(sys.argv) > 2 else "sd15"
    MODEL_SD15 = "runwayml/stable-diffusion-v1-5"
    MODEL_SD2 = "stabilityai/stable-diffusion-2-base"
    single_example_model = MODEL_SD2 if model_arg == "sd2" else MODEL_SD15

    if len(sys.argv) > 1 and sys.argv[1] == "architect":
        # Single example: architect, SD 1.5, seed 455678
        run_single_example_architect(seed=455678)
    elif len(sys.argv) > 1 and sys.argv[1] == "musician":
        # Musician, seeds 36, 72, 87, 88 (default SD 1.5; use "musician sd2" for SD 2.0)
        for seed in [36, 72, 87, 88]:
            run_single_example(
                profession="musician",
                seed=seed,
                model_id=single_example_model,
                output_dir=os.path.join("musician_male_to_female_example", f"seed_{seed}"),
            )
    elif len(sys.argv) > 1 and sys.argv[1] == "pilot":
        # Pilot, seeds 36, 72, 87, 88 (default SD 1.5; use "pilot sd2" for SD 2.0)
        for seed in [36, 72, 87, 88]:
            run_single_example(
                profession="pilot",
                seed=seed,
                model_id=single_example_model,
                output_dir=os.path.join("pilot_male_to_female_example", f"seed_{seed}"),
            )
    elif len(sys.argv) > 1 and sys.argv[1] == "doctor":
        # Doctor, race attribution, red mask, seeds 36, 72, 87, 88 (SD 1.5)
        for seed in [36, 72, 87, 88]:
            run_single_example(
                profession="doctor",
                seed=seed,
                model_id=single_example_model,
                attribution="race",
                output_dir=os.path.join("doctor_race_example", f"seed_{seed}"),
            )
    elif len(sys.argv) > 1 and sys.argv[1] == "mechanic":
        # Mechanic, race attribution, red mask, scenes: kitchen, office, seeds 36, 72, 87, 88 (SD 1.5)
        ...
        for scene in scenes:
            for seed in [36, 72, 87, 88]:
                run_single_example(
                    ...
                    attribution="race",
                    scene=scene,
                    output_dir=os.path.join(f"mechanic_{scene}_race_example", f"seed_{seed}"),
                )
    else:
        # Batch run over professions and seeds
